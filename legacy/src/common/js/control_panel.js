...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function clockSetting() {
            ...
            function postNames() {
				...
				// Clear localStorage if field is empty, otherwise save the value
				if (p1Name.value.trim() === "") {
					localStorage.removeItem("p1NameCtrlPanel");
				} else {
					localStorage.setItem("p1NameCtrlPanel", p1Name.value);
				}

				if (p2Name.value.trim() === "") {
					localStorage.removeItem("p2NameCtrlPanel");
				} else {
					localStorage.setItem("p2NameCtrlPanel", p2Name.value);
				}

				if (raceInfoTxt.value.trim() === "") {
					localStorage.removeItem("raceInfo");
				} else {
					localStorage.setItem("raceInfo", raceInfoTxt.value);
				}

				if (wagerInfoTxt.value.trim() === "") {
					localStorage.removeItem("wagerInfo");
				} else {
            ...
            function postScore(opt1,player) {
				...
				if (opt1 == "add") {
					...
					bc.postMessage(msg);
					// Remove from localStorage if 0, otherwise save
					if (p1ScoreValue === 0) {
						localStorage.removeItem("p"+player+"ScoreCtrlPanel");
					} else {
			...
			function resetScore() {
				...
				p1ScoreValue = 0;
				p2ScoreValue = 0;
				// Remove scores from localStorage instead of setting to 0
				localStorage.removeItem("p1ScoreCtrlPanel");
				localStorage.removeItem("p2ScoreCtrlPanel");
